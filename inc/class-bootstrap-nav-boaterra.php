 ...
 * @version  2.1.4
 */
class Odin_Bootstrap_Boaterra_Nav_Walker extends Walker_Nav_Menu {

	private $product_category = false;
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			$children_text = ' </a>';
			if ( $item->object == 'product_cat' && $depth == 0 && in_array( 'menu-item-has-children', $item->classes ) ) {
				$this->product_category = $item->object_id;
			}

			$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
			if ( 0 === $depth ) {
				$item_output .= '<span>&bull;</span>';
			}
			$item_output .= ( $args->has_children && 0 === $depth ) ? $children_text : '</a>';
